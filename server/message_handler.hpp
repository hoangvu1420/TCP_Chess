...
private:
    ...
    void handleChallengeRequest(int client_fd, const std::vector<uint8_t> &payload)
    {
        ...
        if (server.isUserLoggedIn(message.to_username))
        {
            ...
            uint16_t from_elo = storage.getUserELO(from_username);
            std::string to_username = server.getUsername(to_client_fd);

            ChallengeNotificationMessage notificationMessage;

            notificationMessage.from_username = from_username;
            notificationMessage.elo = from_elo;

            std::vector<uint8_t> serialized = notificationMessage.serialize();
            server.sendPacket(to_client_fd, notificationMessage.getType(), serialized);
        }
        else
        {
